...
function getContent() {
    ...
    if(isUpToDate()) {
        ...
        if (isPreRelease) {
            msg += '<li>'+$.i18n._("You have a pre-release version of Astrax intalled.");
        }
        if (hasPatch) {
            msg += '<li>'+$.i18n._("A patch update for your Astrax installation is available.");
        }
        if (hasMinor) {
            msg += '<li>'+$.i18n._("A feature update for your Astrax installation is available.");
        }
        if (hasMajor && !hasMultiMajor) {
            msg += '<li>'+$.i18n._("A major update for your Astrax installation is available.");
        }
        if (hasMultiMajor) {
            msg += '<li>'+$.i18n._("Multiple major updates for Astrax installation are available. Please upgrade as soon as possible.");
        }
        msg += '</ul>';
    }
